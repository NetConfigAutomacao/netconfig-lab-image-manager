...
def ishare2_install():
    ...
    return (
        jsonify(
            ...
            fallback_attempted=bool(data.get("fallback_attempted")),
            fallback_used=bool(data.get("fallback_used")),
            fallback_prefix=data.get("fallback_prefix", ""),
            fallback_prefixes=data.get("fallback_prefixes", []),
            tested_prefixes=data.get("tested_prefixes", []),
            ranked_prefixes=data.get("ranked_prefixes", []),
            latency_ms=data.get("latency_ms", {}),
            attempt_details=data.get("attempt_details", []),
            quota_detected=bool(data.get("quota_detected")),
            quota_matches=data.get("quota_matches", []),
            status_code=resp.status_code,
        ),
        200,
    )
